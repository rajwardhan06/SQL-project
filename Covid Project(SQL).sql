...
select continent, max(cast(total_deaths as int)) as totaldeathcount
 ...
 --breaking into datewise total cases and totaldeaths
  ...
  select date, isnull(sum(new_cases),0) as total_cases, isnull(sum(cast(new_deaths as int)),0)as total_deaths, isnull(sum(cast(new_deaths as int))/sum(new_cases)*100,0) as deathpercentage
 from PortfolioProject..CovidDeaths
 where continent is not null
 group by date
 order by 1,2
